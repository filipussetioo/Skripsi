...
\section{CodeIgniter 3\cite{ci3:23}}
\label{sec:ci3} 
 
CodeIgniter 3 merupakan sebuah \textit{framework opensource} yang berfungsi untuk mempermudah pengguna dalam membentuk aplikasi \textit{website} menggunakan bahasa PHP. CodeIgniter 3 memiliki tujuan untuk membantu pengguna dalam membentuk aplikasi web lebih cepat dengan menyediakan beragam \textit{library} dan tampilan dan \textit{logic} yang simpel. \textit{CodeIgniter 3} ringan, cepat, menggunakan struktur \textit{Model-View-Controller}, dan menghasilkan \textit{URLs} yang bersih. \textit{Code Igniter 3} memiliki \textit{flow chart} aplikasi yang dapat dilihat pada gambar \ref{fig:ci3flowchart}.
...
\begin{enumerate}
\item \textit{Index.php} berfungsi sebagai \textit{front controller} yang berguna untuk melakukan inisiasi
\item \textit{Router} berfungsi dalam melakukan pengecekan dan menentukan penggunaan \textit{HTTP Request}.
...
\subsubsection{\textit{\textbf{Model}}} berfungsi dalam mewakili struktur data perangkat lunak. \textit{Model} berfungsi dalam mengambil, memasukan, dan memperbarui data pada \textit{database}. Berikut merupakan contoh \textit{file Model} \textit{CodeIgniter 3} pada direktori \textit{application/models/}:
...
Nama \textit{controller} dan metode diatas akan dijadikan segmen pada \textit{URL} seperti berikut:
...
\begin{center}
\item \verb|example.com/news/article/my_article|
\end{center}
...
\item Segmen kedua mereprentasikan kelas fungsi atau method yang digunakan.
...
Secara asali \textit{URL} yang dihasilkan \textit{CodeIgniter 3} terdapat nama \textit{file index.php} seperti contoh dibawah ini:

\begin{center}
\verb|example.com/index.php/news/article/my_article|
\end{center}

Pengguna dapat menghapus \textit{file index.php} file pada \textit{url} menggunakan \textit{file .htaccess} apabila \textit{server Apache} pengguna menghidupkan \textit{mod\_ rewrite}. Berikut merupakan contoh aturan sederhana menggunakan metode \textit{negative}:
...
Pengguna dapat mengubah \verb|enable_query_strings| menjadi \textit{TRUE}.
...
Pengguna selanjutnya harus melakukan inisiasi \textit{library} pada bagian \textit{<head>} seperti berikut:
...
Penggunaan \textit{library email} dapat dikonfigurasikan pada \textit{file config}. Kode \ref{kode:emaillib} merupakan contoh konfigurasi pada \textit{file config}.
...
Selain itu, pengguna juga dapat melakukan konfigurasi preferensi \textit{email} melalui dua puluh satu preferesi. Pengguna dapat melakukan konfigurasi secara otomatis melalui \textit{file config} atau melakukan konfigurasi secara manual . Kode \ref{kode:emaillibset} merupakan contoh konfigurasi secara manual.

\begin{lstlisting}[caption=Contoh konfigurasi \textit{library email} secara manual, label=kode:emaillibset]
...
Kode \ref{kode:controllerfileupload} merupakan contoh kode untuk melakukan validasi dan penyimpanan. Direktori penyimpanan dapat diubah sesuai dengan kebutuhan namun perlu pengubahan izin direktori menjadi 777.
...
\item Parsedown
\item PHPExcel
\item shj\_pagination
\item password\_hash
\item twig
\item profiler ada di ci
\item typography ada di ci
\item encrypt
\end{itemize}
\fi
...
\begin{table}[H] 
	\centering 
	\caption{\textit{Tabel aksi setiap pengguna}}
...
Terdapat dua buah metode untuk melakukan pengecekan yakni metode \textit{Input Output} dan metode \textit{Tester}.
...
\textit{SharIF Judge} menggunakan \textit{Moss} untuk mendeteksi kode yang mirip. \textit{Moss}(\textit{Measure of Software Similarity}) merupakan sistem otomatis untuk menentukan kesamaan atau kemiripan dalam program. Penggunaan utama \textit{Moss} adalah untuk melakukan pengecekan plagiarisme pada kelas \textit{programming}. Pengguna dapat mengirim hasil kode terakhir(\textit{Final Submission}) ke \textit{server Moss} dengan satu klik.
...
\section{Koversi CodeIgniter 3 ke CodeIgniter 4}
...
\subsection{Model, View, and Controller}
...
\subsection{Working with Uploaded Files}
