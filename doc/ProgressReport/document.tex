...
\newcommand{\tanggal}{30/05/2023} %Tanggal pembuatan dokumen
\newcommand{\@judultopik}{Konversi SharIF Judge dari CodeIgniter 3 ke CodeIgnier 4} % Judul/topik anda
\newcommand{\kodetopik}{PAN5392}
\newcommand{\jumpemb}{1} % Jumlah pembimbing, 1 atau 2
\newcommand{\pembA}{Pascal Alfadian Nugroho}
\newcommand{\pembB}{-}
\newcommand{\semesterPertama}{8 - Genap 22/23} % semester pertama kali topik diambil, angka 1 dimulai dari sem Ganjil 96/97
...
\item Bagaimana mengevaluasi kode SharIF Judge dan mengubahnya agar dapat berjalan di Code22 Igniter 4?
\end{itemize}
...
Setelah dilakukan pemanggilan, pengguna dapat memakai fungsi-fungsi yang disediakan sesuai dengan \textit{helpers} yang digunakan.
		...
		{\bf Status :} Ada sejak rencana kerja skripsi.\\
		{\bf Hasil :} \\ Aplikasi akan dikonversi menuju \textit{CodeIgniter 4} dengan struktur sebagai berikut.
		\begin{figure}[H]
			\centering  
...
Pengguna tidak membutuhkan direktori \textit{core} dan tidak membutuhkan kelas \verb|MY_X| pada direktori \textit{libraries} untuk memperpanjang atau mengganti potongan \textit{CI4}. Pengguna dapat membentuk kelas dimanapun dan menambahkan metode pada \verb|app/Config/Services.php|.
		...
		{\bf Hasil :} \textit{Twig} merupakan sebuah \textit{template engine} untuk bahasa pemrograman \textit{PHP}. \textit{Twig} digunakan pada \textit{view} aplikasi \textit{ShaIF Judge} untuk menampilkan data berupa variabel dan melanjutkan atau memasukan sebuah \textit{template view} pada \textit{view} lainnya. Kode \ref{kode:twigtag} merupakan contoh penggunaan \textit{Twig} pada salah satu halaman \textit{SharIF Judge}.
...
<ul>
    ...
    {% endfor %}
		...
		{\bf Status :} baru ditambahkan pada semester ini\\
		{\bf Hasil :} Mengimplementasikan kode pada projek \textit{CodeIgniter 4} antara lain fitur \textit{install, login, logout, dashboard, profile} ,dan \textit{setting}. Berikut merupakan fitur yang sudah di implementasikan:
		\begin{itemize}
		 	\item \textbf{Login}
		 	\begin{figure}[H]
	\centering  
